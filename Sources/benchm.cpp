...
int benchmark(bool excess, bool defect, int num_nodes, double  average_k, int  max_degree, double  tau, double  tau2, 
	double  mixing_parameter, int  overlapping_nodes, int  overlap_membership, int  nmin, int  nmax, bool  fixed_range, double ca, bool powerLaw) {	
	...
	if(powerLaw){
		...
		cout<<"barabasssssiii"<<endl;
		barabasi_albert_cumulative_distribution(max_degree,average_k, cumulative);
	}
...
int main(int argc, char * argv[]) {
	...
	benchmark(p.excess, p.defect, p.num_nodes, p.average_k, p.max_degree, p.tau, p.tau2, p.mixing_parameter, p.overlapping_nodes, p.overlap_membership, p.nmin, p.nmax, p.fixed_range, p.clustering_coeff, p.powerLaw);	
