...
int barabasi_albert_cumulative_distribution (int n, int average_k, deque<double> &cumulative) {
	cumulative.clear();
	int m = .05f * average_k;			
	float degrees[n] {0};
	float sum = m * m;
	...
	// Add node to graph with m links to nodes
	for (int i = m; i < n; i++)
	{
		int found = 0;
		int rollingSum = 0;
		int connectNewNodeTo[m] {-1};
		while (found < m){
			// Connect with a node
			int cumul = 0;
			float r = 0.01f * (float)rand() * rollingSum;
			for (int u = 0; u < i; u++) // Pass nodes to check if connect
			{
				cumul += degrees[u];
				if(cumul > r){
					...
					{
						if(connectNewNodeTo[j] == u){
							// Node already selected to connect. Please try again
							notDouble = false;
							break;
						}
					}
					if(notDouble){
						connectNewNodeTo[found] = u;
						found++;
					}
					break;
				}
		...
		// Add node and update degrees
		degrees[i] = m;
		for (int newC = 0; newC < m; newC++)
		{
			degrees[connectNewNodeTo[newC]]++;
		}
	}
	// Save degrees in cumulative distribution
	int degreeCounter = 0;
	for (int i = 0; i < n; i++)
	{
		degreeCounter += degrees[i];
		cumulative.push_back(degreeCounter);
	}
